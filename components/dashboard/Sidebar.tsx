        ...
        <div className={cn("p-6 h-full flex flex-col", sidebarCollapsed ? 'px-1 py-4' : '')}>
          ...
          <nav className="space-y-2 mb-8 flex-1">
            {sidebarItems.map((item, index) => (
              <div key={index} className={`sidebar-item ${item.active ? 'active' : ''}`}>
                {loading || !currSite ? (
                  <span className={`flex items-center gap-${sidebarCollapsed ? '0' : '3'} px-0 pl-${sidebarCollapsed ? '0' : '4'} py-2 rounded-lg text-[var(--text-secondary)] transition-all duration-200 opacity-50 cursor-not-allowed ${sidebarCollapsed ? 'justify-center' : ''}`}>
